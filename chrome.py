...
class ErrorItem(alfred.Item):
    def __init__(self, error):
        alfred.Item.__init__(self, {'valid': 'NO', 'autocomplete': str(error)}, str(error), 'Check the workflow log for more information.')
...
def history_db(profile, favicons=True):
    ...
    if favicons:
        favicons_db = copy_db(FAVICONS_DB, profile)
        db.cursor().execute('ATTACH DATABASE ? AS favicons', (favicons_db,)).close()
    return db
...
def history_results(db, query, favicons=True):
    ...
    for row in db.execute(HISTORY_QUERY.format(favicon_select=favicon_select, favicon_join=favicon_join), (q, q,)):
        ...
        yield alfred.Item({'uid': alfred.uid(uid), 'arg': url, 'autocomplete': url}, title or url, url, icon)
